...
export class SkinCareComponent implements OnInit {
  ...
  loadSkinCareProducts(): void {
    this.productService.getProductsByCategory(this.productCategoryId, this.currentPage, this.pageSize).subscribe(
      (response) => {
        console.log('Loaded Skin Care products:', response);
      ...
      (error) => {
        console.error('Error loading Skin Care products:', error);
      }
    );
  }
  ...
  showMessage(message: string) {
    this.snack.open(message, 'OK', {
      duration: 4000,
      panelClass: ['snackbar-error'], // za stilizaciju
  ...
  goToDetails(id: number){
    this.router.navigate(['/skin-product-details', id]);
  }
